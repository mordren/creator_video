...
class TTSManager:
    """Gerenciador central de TTS - Versão simplificada e modular"""
    
    def _carregar_texto_do_roteiro(self, roteiro_path: Path, config: Dict[str, Any]) -> tuple:
        """Carrega o texto do roteiro de forma compatível com seu código atual"""
        try:
            with open(roteiro_path, 'r', encoding='utf-8') as f:
                dados = json.load(f)
            
            # Encontra o texto para sintetizar (mantendo compatibilidade)
            idioma = dados.get('idioma', config.get('IDIOMA', 'pt'))
            chave_texto = f"texto_{idioma}"
            
            # Fallback para chaves comuns (seu código atual)
    ...
    def gerar_audio(self, roteiro_path: Path, canal: str, provider: str = None) -> bool:
        """Gera áudio para um roteiro - Versão modularizada"""
        ...
        # Prepara caminho de saída (compatível com seu código atual)
